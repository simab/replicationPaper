...
an.df.rugged$pys <- an.df.rugged$peaceyears
an.df.rugged$pys2 <- an.df.rugged$peaceyears^2
an.df.rugged$pys3 <- an.df.rugged$peaceyears^3
...
an.df.rugged.sub.allyears <- an.df.rugged %>%
  filter(isrelevant==1,
         status_monopoly==0,
         status_dominant==0,
         !is.na(seg_area_sqkm),
...
for(treat_var in treat.vars){
  ...
  onset_logit <- glm(onset_do_flag ~ ., 
                     data=an.df.rugged.sub.allyears[,full_lhs], family="binomial")
...
logit_split_coefs <- coeftest(logit_split, 
                              vcov. = vcovCL(logit_split, 
                                             cluster = an.df.rugged.sub.allyears$ag_id))
...
stargazer(logit_split_coefs, logit_tfrac_coefs, logit_tfrac_1946_coefs,
          ...
          covariate.labels = c("Divided group", "Fractionalization", 
                               ...
                               "Terrain Ruggedness","Relative size", "War history",
...
logit_2_prepost1946 <- glm(onset_do_flag ~ ., 
                      data = an.df.rugged.sub.allyears[,combined_lhs], family="binomial")                               
...
for(treat_var in treat.vars.3){
  ...
  d <- an.df.rugged.sub.allyears[,full_lhs]
  if(treat_var == "tfrac"){
...
pred_prob_post1946_onset_logit <- glm(onset_do_flag ~ ., 
                   data=an.df.rugged.sub.allyears[,pred_prob_lhs], family="binomial")                               
...
mean_var_vals <- c(1, map(an.df.rugged.sub.allyears[,pred_prob_lhs[3:17]], function(x) mean(x, na.rm=TRUE)))
...
max_tfrac_post1946 <- max(an.df.rugged.sub.allyears$tfrac_incr_post1946, na.rm=TRUE)
